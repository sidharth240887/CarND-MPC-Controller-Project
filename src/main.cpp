      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // preprocessing.
          // transforms waypoint cord to the car cord.
          size_t n_waypnts = ptsx.size();
          auto ptsx_trans = Eigen::VectorXd(n_waypnts);
          auto ptsy_trans = Eigen::VectorXd(n_waypnts);
          for (unsigned int k = 0; k < n_waypnts; k++ ) {
            double d_X = ptsx[k] - px;
            double d_Y = ptsy[k] - py;
            double minus_psi = 0.0 - psi;
            ptsx_trans( k ) = d_X * cos( minus_psi ) - d_Y * sin( minus_psi );
            ptsy_trans( k ) = d_X * sin( minus_psi ) + d_Y * cos( minus_psi );
          }
